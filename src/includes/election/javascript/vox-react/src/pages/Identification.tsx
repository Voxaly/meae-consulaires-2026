...
const Identification = () => {
    ...
    useEffect(() => {
        if (captchaRef.current && captcha.captchaId === '') {
            setCaptcha((prev) => ({
                ...prev, captchaId: captchaRef.current.getCaptchaId()
            }))
        }
    }, [captchaRef.current])
    ...
    const isFormValid = useCallback(() => {
        ...
        const filteredErrors = Object.fromEntries(
            Object.entries(errors).filter(([key]) => key !== 'global')
        );
    ...
    // ----- MOTEUR LORIA POUR CHIFFREMENT DU BULLETIN TÉMOIN ------ //
    ...
    const initEngine = useCallback((force?: boolean) => {
        ...
        if (engine.engineTemoin === null && data?.jsConfigCryptoTemoinJSON !== undefined) {
            ...
            engine.engineTemoin.init(JSON.parse(unescape(data.jsConfigCryptoTemoinJSON)));
        }
        setEngine(engine);
    ...
    useEffect(() => {
        if (engine.engineTemoin !== null && data !== undefined) {
            (async function () {
                const temoin = unescape(data.temoin).split(',').map((val) => +val);
        ...
        <VoxMain type={"colonne"}>
            ...
            {errors && errors['global'] && (
                <div className="fr-alert fr-alert--error fr-mb-2w" role="alert">
                    {getText(errors['global'], globalData?.common.textes, [])}
                </div>
            )}
            {data?.error && data?.error.message && (
                <div className="fr-alert fr-alert--error fr-mb-2w" role="alert">
                    {getText(data?.error.message, globalData?.common.textes, [])}
                </div>
            )}
