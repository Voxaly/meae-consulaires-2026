...
import {unescape} from "../../utils/string.util";
...
const ConfirmationVote = () => {
    ...
    const {jsConfigCryptoJSON, voteStatus, stepperState, parameters, textes} = useNavigationClientContext();
    ...
    const onSubmit = async (e: React.FormEvent<EventTarget>) => {
        ...
        shaBulletinGenerator.update(state.bulletinChiffre + input);
        const shaHashBulletin = shaBulletinGenerator.getHash('HEX');
        setState({...state, voteSignature: input, hashBulletin: shaHashBulletin, correctLength: true})
        ...
        if (isElecteurFIN) {
            ...
            checkCodeActivation({
                form: {
                    bulletin: state.bulletinChiffre,
                    hashBulletin: shaHashBulletin,
                }
            });
        }

        setState({...state, aVote: true});
    }
    ...
    const sendVote = () => {
        postVote({
            form: {
                bulletin: state.bulletinChiffre,
                cryptoLibrary: state.cryptoLibrary,
                clientInfos: state.clientInfos,
                idElection: election.id,
                hashBulletin: state.hashBulletin,
                voteSignature: state.preuveLegitimite,
            }
        })
    }
    ...
    useEffect(() => {
        ...
        // Initialisation du Moteur Loria
        const engine = new JsEncryptionEngine('MAIN');
        engine.init(JSON.parse(unescape(jsConfigCryptoJSON)));
        ...
        return (
            <>
                ...
                <form autoComplete="off" onSubmit={onSubmit}>
                    ...
                    <BoutonVote disabled={disableSubmitBtn}/>
                </form>
            </>
        )
    } else return (
...
const BoutonVote = ({disabled}: { disabled: boolean }) => {
    const {textes} = useNavigationClientContext();
    ...
    return (
        <ButtonsGroup
            ...
            buttons={[
                {
                    type: 'submit',
                    children: getText('confirmation-vote.bouton.voter', textes),
                    disabled,
                }
            ]}
        />
