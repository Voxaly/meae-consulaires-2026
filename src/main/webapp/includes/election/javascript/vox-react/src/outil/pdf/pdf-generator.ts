...
export class PdfGeneratorService {
    ...
    async generateRecepissePdf(): Promise<void> {
        ...
        try {
            ...
            // 3. Logo vote (SVG - respect du ratio original via svg2pdf)
            const logoTargetHeight = 18; // hauteur cible en mm
            try {
                const parser = new DOMParser();
                const svgDoc = parser.parseFromString(textes.logoVote, 'image/svg+xml');
                const svgEl = svgDoc.querySelector('svg') as SVGSVGElement;

                let logoW = logoTargetHeight;
                ...
                if (svgEl) {
                    ...
                    if (viewBox) {
                        ...
                        if (parts.length === 4) {
                            ...
                            if (vbH > 0) {
                                logoW = logoTargetHeight * (vbW / vbH);
                            }
                        }
                    }
                    await svg2pdf(svgEl, doc, {
