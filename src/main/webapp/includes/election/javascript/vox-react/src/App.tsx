...
const Root = () => {
    ...
    useEffect(() => {
        if (!isLoading && globalData) {
            ...
            } else if (globalData.common.etatScrutin === EtatScrutin.RESULTATS
                || (globalData.common.etatScrutin === EtatScrutin.VOTE_CLOS && !isSessionLoading && !isConnected)) {
                document.location.href = URL_VOTE_CLOS;
            } else if (globalData.common.etatScrutin === EtatScrutin.VOTE_CLOS && !isSessionLoading
                && isConnected && !globalData.common.dansDelaiDeGrace) {
                // Délai de grâce écoulé alors que la session a survécu (pages hors filtre de période) :
                // déconnexion, dont la chaîne serveur redirige ensuite vers vote-clos.
                document.location.href = URL_LOGOUT;
            }
        }
    }, [isLoading, isSessionLoading, globalData, isConnected]);
    ...
    // Rafraîchissement pendant le délai de grâce : la session serveur est préservée alors que
    // le scrutin est clos. On rend l'application normalement (l'électeur reste sur sa page)
    // et on lui présente la même modale de déconnexion que le bouton « Se déconnecter » :
    // s'il refuse la déconnexion, la modale se ferme et il poursuit sa session de vote.
    const isDelaiGrace = !isLoading && !isSessionLoading
        && globalData?.common.etatScrutin === EtatScrutin.VOTE_CLOS && isConnected
        && globalData.common.dansDelaiDeGrace;

    if (isLoading || (globalData?.common.etatScrutin !== EtatScrutin.EN_COURS && !isDelaiGrace)) return null;
    else return (
        <AxiosInterceptor>
            <GlobalProvider global={globalData!}>
                <ElectionProvider>
                    ...
                    {isConnected && (
                        <VoxModalLogout openOnMount={isDelaiGrace}/>
                    )}
                    {isConnected &&
