...
import {CodeErreurElecteur, logErreurElecteur} from "../../utils/codeErreurElecteur";
...
interface VoteState {
    ...
    hashBulletin: string;
    codeErreurConstruction?: CodeErreurElecteur;
}
...
const ConfirmationVote = () => {
    ...
    useEffect(() => {
        ...
        (async function () {
            ...
            // Chaque étape de la construction du bulletin a son propre code d'erreur représentatif
            // (CodeErreurElecteur), pour pouvoir corréler le message affiché à l'électeur avec sa
            // cause exacte dans les logs navigateur, même en l'absence de remontée serveur.
            let ballot;
            try {
                ...
                logErreurElecteur(CodeErreurElecteur.BULLETIN_CONSTRUCTION_MOTEUR_CRYPTO, error);
                setState({...state, choixCorrectementForme: false, codeErreurConstruction: CodeErreurElecteur.BULLETIN_CONSTRUCTION_MOTEUR_CRYPTO});
                return;
            }

            const storage = sessionStorage;
            const skSession = storage.getItem(ballotSignatureKey); // Généré par vote_api.mjs lors de la connexion
            if (!skSession) {
                logErreurElecteur(CodeErreurElecteur.BULLETIN_CONSTRUCTION_CLE_SESSION, new Error('Clé de session de signature du bulletin absente'));
                setState({...state, choixCorrectementForme: false, codeErreurConstruction: CodeErreurElecteur.BULLETIN_CONSTRUCTION_CLE_SESSION});
                return;
            }
            ...
            try {
                ...
                electorModule.recoverSk(skSession);
                ...
                logErreurElecteur(CodeErreurElecteur.BULLETIN_CONSTRUCTION_SIGNATURE, error);
                setState({...state, choixCorrectementForme: false, codeErreurConstruction: CodeErreurElecteur.BULLETIN_CONSTRUCTION_SIGNATURE});
                return;
            }
            ...
            try {
                ...
                logErreurElecteur(CodeErreurElecteur.BULLETIN_CONSTRUCTION_INCONNUE, error);
                setState({...state, choixCorrectementForme: false, codeErreurConstruction: CodeErreurElecteur.BULLETIN_CONSTRUCTION_INCONNUE});
            }
        })();
    }, []);
    ...
    if (!state.choixCorrectementForme) {
        return (
            <div className="fr-alert fr-alert--error fr-mb-3w" role="alert">
                <p className="fr-text--bold">
                    {getText('confirmation-vote.erreur-construction-bulletin', textes)}
                    {state.codeErreurConstruction ? ` (${state.codeErreurConstruction})` : ''}
                </p>
            </div>
        );
        ...
        return (
            <>
                <AlertVoteDefinitif/>
                {/* Conteneur permanent pour les erreurs globales (RGAA 7.5) */}
                <div role="alert" aria-atomic="true" className="fr-mb-3w">
                    {errors['global'] && (
            ...
            <form autoComplete="off" onSubmit={onSubmit}>
                <DSFRInput
                    ...
                    texts={{
                        ...
                        passwordCheckBoxLabel: getText('confirmation-vote.champ.code.afficher.label', textes),
                        passwordCheckboxAriaLabel: getText('confirmation-vote.champ.code.afficher.ariaLabel', textes),
