...
const ConfirmationChoix = () => {
    ...
    const {mutate: sendActivationCode} = usePostCodeActivation({
        onSuccess: (data) => {
            if (data.data === "ok") {
                ...
                setErrors({global: getText('ERR-VOTE-ACTIVATION', textes)});
            }
        },
        onError: (err) => {
            setState({confirmed: false});
            errorHandler(err as any, setErrors, {...textes, ...globalData?.common.textes});
        },
    });
    ...
    return (
        <form id="voteForm" name="voteForm" method="post" autoComplete="off">
            ...
            {/* Conteneur permanent pour les messages de statut (RGAA 7.5) */}
