...
import {URL_ERREUR, URL_VOTE_CLOS} from "../config/const";
    ...
    useEffect(() => {
        if (redirect !== "") {
            if (redirect.endsWith("erreur")) {
                navigate(URL_ERREUR);
            } else if (forceReload || redirect === URL_VOTE_CLOS) {
                // Chargement complet : soit pour récupérer un nouveau token CSRF après invalidation
                // de session, soit parce que la cible est hors React (vote-clos) et que navigate()
                // la ferait tomber sur la route 404.
