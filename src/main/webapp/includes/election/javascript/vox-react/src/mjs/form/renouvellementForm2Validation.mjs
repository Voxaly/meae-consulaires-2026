...
initState(loginInput, 'loginInput', REGEX_LOGIN_PASSWORD);
initState(passwordInput, 'passwordInput', REGEX_LOGIN_PASSWORD);
initState(captchaInput, 'captchaDetect', /\d{4}/);
isRadioChecked(canalReception, 'canalReception');
updateSubmitButton();

/**
 * Initialise l'état d'un champ à partir de sa valeur courante dans le DOM
 * @param {HTMLInputElement} input
 * @param {'loginInput', 'passwordInput', 'captchaDetect'} inputId
 * @param {RegExp} regex
 */
function initState(input, inputId, regex) {
    if (!input) return;
...
function isRadioChecked(radioGroup, radioGroupId) {
    if (!radioGroup) return;
    const radios = radioGroup.querySelectorAll('input[type="radio"]');
    state[radioGroupId] = Array.from(radios).some((radio) => radio.checked);
}
