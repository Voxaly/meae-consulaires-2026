            ...
            {type === "password" && (
                <div className="fr-password__checkbox fr-checkbox-group fr-checkbox-group--sm">
                    ...
                    <label className="fr-password__checkbox fr-label" htmlFor={`${id}-show`}>
                        ...
                        {texts.passwordCheckboxAriaLabel && (
                            <span className="fr-sr-only">&nbsp;{texts.passwordCheckboxAriaLabel}</span>
                        )}
                    </label>
                </div>
            )}
            {/* Message d'erreur — le conteneur doit toujours exister dans le DOM (RGAA 7.5) */}
            <div id={`${id}-messages`} className="fr-messages-group" aria-live="assertive" aria-atomic="true">
                {!!errors?.[id] && (
                    <p id={`${id}-errors`} className={"fr-message fr-message--error"}>
                        {errors[id]}
                    </p>
                )}
            </div>
